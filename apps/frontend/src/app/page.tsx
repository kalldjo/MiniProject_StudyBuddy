                  ...
                  <div key={op.id} className="p-3 bg-white border border-zinc-100 rounded-2xl relative flex flex-col gap-2 shadow-[0_2px_8px_rgba(0,0,0,0.01)] hover:scale-[1.01] transition-transform">
                    ...
                    <div className="flex items-center justify-between border-t border-zinc-50/80 pt-2 mt-1">
                      ...
                      <button 
                        onClick={async () => {
                          try {
                            await apiFetch(`/opportunities/${op.id}/apply`, { method: 'POST' });
                            setAppliedRole(`${op.role} at ${op.company}`);
                            setShowApplyModal(true);
                            setOpportunities(prev => prev.map(o => o.id === op.id ? { ...o, hasApplied: true } : o));
                          } catch (err) {
                            console.error('Gagal melamar lowongan:', err);
                            alert('Gagal mengirimkan lamaran pekerjaan');
                          }
                        }}
                        disabled={op.hasApplied}
                        className={`text-[9px] font-extrabold px-2.5 py-1 rounded-lg transition shadow-sm ${op.hasApplied ? 'bg-zinc-100 text-zinc-400 border border-zinc-200 cursor-not-allowed shadow-none' : 'bg-logo-gradient text-white hover:opacity-90'}`}
                      >
                        {op.hasApplied ? 'Applied' : 'Apply'}
                      </button>
