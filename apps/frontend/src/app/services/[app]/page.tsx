...
export default function ServicesPage() {
  ...
  const [groupMessages, setGroupMessages] = useState<Record<string, GroupMessage[]>>({
    ...
    "2": [{ sender: "Jo", text: "Grup algoritma graf sudah aktif. Silakan share flowchart-nya.", time: "10:05" }],
  });
  ...
  const handleTriggerSummary = () => {
    ...
    setTimeout(() => {
      ...
      setSummaryOutput({
        ...
        bullets: ["Neo4j menyimpan pointer relationship secara fisik (Index-free Adjacency) sehingga navigasi join sangat cepat.", "Penggunaan kueri MATCH (a:User)-[:FRIEND]->(b:User) tidak memerlukan tabel perantara seperti RDBMS.", "Gunakan perintah CREATE INDEX FOR (n:User) ON (n.name) untuk mempercepat pencarian node awal.", "Hindari traversals tanpa arah jika skema bersifat direksional untuk menghemat memori pencarian."],
      });
    }, 1500);
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 pb-12 pt-6">
      <div className="max-w-6xl w-[95%] mx-auto flex flex-col gap-6">
        ...
        <div className="p-2 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-2">
            <h1 className="text-3xl font-extrabold text-[#1D1D1F] tracking-tight">StudyBuddy Premium Tools</h1>
            <p className="text-sm text-zinc-400 font-semibold max-w-xl">Explore 4 state-of-the-art interactive micro-applications for UI students</p>
          </div>
          <Link href="/">
            <button className="px-5 py-2 text-xs font-black bg-zinc-100 hover:bg-zinc-200 border border-zinc-200 text-zinc-700 rounded-full transition">Back to Feed</button>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 p-5 rounded-3xl bg-white border border-zinc-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {navItems.map((item) => (
                <button key={item.id} onClick={() => router.push(`/services/${item.id}`)} className={`flex items-center gap-3 px-3 py-2.5 rounded-2xl text-xs font-bold text-left transition-all ${activeApp === item.id ? "bg-logo-gradient text-white shadow-md scale-[1.01]" : "hover:bg-zinc-50 text-zinc-600 hover:text-[#1D1D1F]"}`}>
                  <span>{item.icon}</span>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex flex-col gap-2 border-r border-zinc-100 pr-2">
                    ...
                    {circles.map((c) => (
                      <button key={c.id} onClick={() => setActiveCircleId(c.id)} className={`p-3 rounded-2xl text-left text-xs font-bold transition ${activeCircleId === c.id ? "bg-indigo-50 border border-indigo-150 text-indigo-700" : "hover:bg-zinc-50 text-zinc-600"}`}>
                  ...
                  <div className="md:col-span-2 flex flex-col justify-between h-[360px] bg-zinc-50 border border-zinc-150 rounded-2xl p-4">
                    ...
                    <div className="flex items-center justify-between border-b border-zinc-200 pb-2 mb-2">
                      <span className="text-xs font-black text-zinc-700">#{circles.find((c) => c.id === activeCircleId)?.name}</span>
                      <button onClick={() => handleJoinCircle(activeCircleId)} className={`px-3 py-1 rounded-full text-[10px] font-extrabold ${circles.find((c) => c.id === activeCircleId)?.joined ? "bg-red-50 text-red-600 border border-red-200" : "bg-indigo-600 text-white"}`}>
                    ...
                    <div className="flex-1 overflow-y-auto flex flex-col gap-3 py-2 pr-1 custom-scrollbar">
                      {(groupMessages[activeCircleId] || []).map((msg, i) => (
                        <div key={i} className={`flex flex-col max-w-[80%] ${msg.sender === "Anda" ? "self-end items-end" : "self-start items-start"}`}>
                          ...
                          <div className={`p-2.5 rounded-2xl text-xs mt-0.5 font-semibold ${msg.sender === "Anda" ? "bg-indigo-600 text-white rounded-tr-none" : "bg-white border border-zinc-200 text-zinc-700 rounded-tl-none"}`}>{msg.text}</div>
                    ...
                    <form onSubmit={handleSendMessage} className="flex gap-1.5 mt-2">
                      <input type="text" placeholder={circles.find((c) => c.id === activeCircleId)?.joined ? "Tulis pesan diskusi..." : "Gabung ke channel untuk berdiskusi"} disabled={!circles.find((c) => c.id === activeCircleId)?.joined} value={typedMessage} onChange={(e) => setTypedMessage(e.target.value)} className="flex-1 p-2 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-indigo-500 font-semibold disabled:bg-zinc-100 disabled:cursor-not-allowed" />
                      <button type="submit" disabled={!circles.find((c) => c.id === activeCircleId)?.joined} className="px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                        Send
                      </button>
                    </form>
                  </div>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="p-4 bg-indigo-50 border border-indigo-150 rounded-2xl flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-semibold text-indigo-700 leading-normal mt-0.5">Sertifikat Neo4j Certified Professional menaikkan nilai tawar HRD hingga 35%!</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                  {jobs.map((j) => (
                    <div key={j.id} className="p-5 bg-white border border-zinc-150 rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-4 shadow-sm hover:border-zinc-250 transition-colors">
                      <div>
                        <span className="px-2 py-0.5 text-[9px] font-extrabold bg-violet-50 text-violet-600 border border-violet-150 rounded-md">{j.lab}</span>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="text-[10px] font-bold text-zinc-400">{j.slots > 0 ? `${j.slots} Slot Tersisa` : "Slot Penuh"}</span>
                        <button onClick={() => (j.applied ? null : setApplyModalJob(j))} disabled={j.applied || j.slots === 0} className={`px-4 py-2 rounded-xl text-xs font-black transition ${j.applied ? "bg-zinc-100 text-zinc-400 cursor-not-allowed border border-zinc-200" : "bg-violet-600 hover:bg-violet-700 text-white shadow-sm"}`}>
                ...
                {applyModalJob && (
                  <div className="p-5 bg-zinc-50 border border-violet-150 rounded-2xl flex flex-col gap-3 relative">
                    <button onClick={() => setApplyModalJob(null)} className="absolute right-3 top-3 text-zinc-400 hover:text-zinc-600 font-bold">
                      ✕
                    </button>
                    ...
                    <form onSubmit={handleApplyJob} className="flex flex-col gap-2.5">
                      <input type="text" placeholder="NPM Mahasiswa (misal: 2206012345)" value={applyText.studentId} onChange={(e) => setApplyText({ ...applyText, studentId: e.target.value })} className="p-2 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-violet-500 font-bold" required />
                      <textarea placeholder="Tuliskan pengalaman basis data Anda (misal: Pernah membuat rancangan normalisasi 3NF dan query Cypher di mini project)..." value={applyText.coverLetter} onChange={(e) => setApplyText({ ...applyText, coverLetter: e.target.value })} className="p-2.5 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-violet-500 font-semibold h-20 resize-none" required />
                  ...
                  {tutoringOffers.map((o) => (
                    <div key={o.id} className="p-4 bg-white border border-zinc-150 rounded-2xl flex flex-col justify-between gap-3 shadow-sm hover:scale-[1.005] transition-transform">
                      ...
                      <div className="flex items-center justify-between border-t border-zinc-100 pt-3 mt-1">
                        ...
                        <button onClick={() => alert(`🎉 Permintaan tutor untuk ${o.title} telah dikirim ke WhatsApp ${o.provider}! Terapkan jadwal segera.`)} className="px-3.5 py-1.5 text-[10px] font-black bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition">
                          Booking Tutor
                        </button>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="md:col-span-2 flex flex-col gap-3">
                    <div className="bg-zinc-900 aspect-video rounded-2xl flex items-center justify-center text-white relative overflow-hidden shadow">
                      <iframe src={`https://www.youtube.com/embed/${chapters[activeChapter].videoId}?rel=0&modestbranding=1`} title={chapters[activeChapter].title} className="absolute inset-0 w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
                    </div>

                    <span className="text-xs font-black text-zinc-400 px-1 mt-1">Penjelasan Slide Ringkas</span>
                    <p className="p-3 bg-zinc-50 border border-zinc-150 rounded-xl text-xs font-semibold text-zinc-600 leading-relaxed">{chapters[activeChapter].notes}</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      {chapters.map((ch, idx) => (
                        <button key={idx} onClick={() => setActiveChapter(idx)} className={`p-2.5 rounded-xl text-left text-xs font-bold transition ${activeChapter === idx ? "bg-indigo-50 border border-indigo-150 text-indigo-700" : "hover:bg-zinc-50 text-zinc-600"}`}>
                    ...
                    <form onSubmit={handleAddNote} className="p-3 bg-zinc-50 border border-zinc-150 rounded-xl flex flex-col gap-2">
                      ...
                      <textarea placeholder="Tulis ringkasan pelajaran..." value={lectureNotes} onChange={(e) => setLectureNotes(e.target.value)} className="p-2 text-[10px] bg-white border border-zinc-200 rounded-lg focus:outline-none focus:border-indigo-500 font-semibold h-16 resize-none" />
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="p-3 bg-zinc-50 border border-zinc-150 rounded-2xl flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input type="range" min="2" max="12" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-20" />
                    <span className="text-xs text-zinc-600 font-bold font-mono">{brushSize}px</span>
                  </div>

                  <div className="flex gap-2">
                    <button onClick={() => setIsEraser(!isEraser)} className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition ${isEraser ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-white hover:bg-zinc-50 text-zinc-500 border-zinc-200"}`}>
                      🧹 Eraser Mode
                    </button>
                    <button onClick={clearCanvas} className="px-3 py-1.5 text-xs font-bold bg-zinc-200 hover:bg-zinc-300 text-zinc-700 rounded-lg transition">
                      Clear Board
                    </button>
                  </div>
                </div>
                ...
                <div className="border border-zinc-150 rounded-3xl overflow-hidden bg-white shadow-inner flex items-center justify-center p-2">
                  <canvas ref={canvasRef} width={600} height={300} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} className="bg-white cursor-crosshair border border-dashed border-zinc-100 rounded-2xl max-w-full" />
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="flex flex-col gap-3">
                    ...
                    <textarea placeholder="Ketikkan atau paste transkrip kuliah di sini (misal: RDBMS butuh join table yang lambat kalau datanya jutaan. Model graf Neo4j menggunakan model traversal direct pointer)..." value={summaryInput} onChange={(e) => setSummaryInput(e.target.value)} className="p-3 text-xs bg-zinc-50 border border-zinc-150 rounded-2xl focus:outline-none focus:border-indigo-500 font-semibold h-44 resize-none w-full" />
                    <button onClick={handleTriggerSummary} disabled={isSummarizing || !summaryInput.trim()} className="w-full py-2.5 bg-logo-gradient text-white text-xs font-black rounded-xl hover:opacity-95 shadow-sm transition disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {summaryOutput ? (
                      <div className="p-5 bg-indigo-50/50 border border-indigo-100 rounded-3xl flex flex-col gap-3 shadow-inner">
                        <span className="px-2 py-0.5 text-[8px] font-black uppercase bg-indigo-100 text-indigo-700 rounded-md self-start">AI CONCEPT MATRIX</span>
                      ...
                      <div className="flex-1 border border-dashed border-zinc-200 rounded-3xl flex flex-col items-center justify-center p-6 text-center text-zinc-400 min-h-[180px]">
                        ...
                        <p className="text-[10px] font-bold mt-2">
                          Belum ada output ringkasan.
                          <br />
                          Isi catatan kasar kuliah di kiri lalu klik ringkas.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            {activeApp === "flashcards" && (
              <div className="flex flex-col gap-6">
                <div className="flex items-start justify-between border-b border-zinc-100 pb-4">
                  ...
                  <button onClick={() => setShowAddCardForm(!showAddCardForm)} className="shrink-0 px-3 py-1.5 text-[10px] font-extrabold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition">
                ...
                {showAddCardForm && (
                  <form onSubmit={handleSaveFlashcard} className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex flex-col gap-2.5 animate-in fade-in duration-200">
                    ...
                    <textarea placeholder="Pertanyaan / Soal (misal: Apa itu Index-Free Adjacency?)" value={newCardQuestion} onChange={(e) => setNewCardQuestion(e.target.value)} className="p-2.5 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-indigo-500 font-semibold h-16 resize-none" required />
                    <textarea placeholder="Jawaban (misal: Teknik Neo4j menyimpan pointer relasi langsung di node...)" value={newCardAnswer} onChange={(e) => setNewCardAnswer(e.target.value)} className="p-2.5 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-indigo-500 font-semibold h-16 resize-none" required />
                ...
                <div className="flex flex-col items-center gap-5">
                  {flashcards.length === 0 ? (
                    ...
                    <>
                      {/* Card Flip Body */}
                      <div onClick={() => setIsCardFlipped(!isCardFlipped)} className="w-full max-w-md h-52 bg-white border border-zinc-200 rounded-3xl p-6 shadow-md flex flex-col justify-between cursor-pointer hover:border-indigo-400 transition-all select-none hover:shadow-lg">
                        <div className="flex justify-between items-center text-[9px] font-black text-zinc-400">
                          <span>
                            KARTU {currentCardIdx + 1} DARI {flashcards.length}
                          </span>
                          <span className="px-2 py-0.5 rounded bg-indigo-50 text-indigo-600 uppercase">{isCardFlipped ? "JAWABAN (CLICK FLIP)" : "PERTANYAAN (CLICK FLIP)"}</span>
                        </div>

                        <div className="flex-1 flex items-center justify-center text-center p-2">
                          <p className={`text-xs leading-relaxed font-black ${isCardFlipped ? "text-indigo-600" : "text-zinc-800"}`}>{isCardFlipped ? flashcards[currentCardIdx].answer : flashcards[currentCardIdx].question}</p>
                        </div>

                        <div className="text-center text-[9px] font-bold text-zinc-400">💡 Klik di mana saja pada kartu untuk membalik posisi.</div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 w-full max-w-md">
                        ...
                        <button onClick={() => handleDeleteFlashcard(flashcards[currentCardIdx].id)} className="flex-1 py-1.5 bg-red-50 hover:bg-red-100 text-red-500 border border-red-200 rounded-xl text-[10px] font-black transition">
                          🗑 Hapus Kartu Ini dari DB
                        </button>
                        <button
                          onClick={() => {
                            setIsCardFlipped(false);
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <form onSubmit={handleAddGrade} className="p-4.5 bg-zinc-50 border border-zinc-150 rounded-2xl flex flex-col gap-3">
                    ...
                    <input type="text" placeholder="Nama Matkul (misal: SBD UI)" value={newCourseName} onChange={(e) => setNewCourseName(e.target.value)} className="p-2.5 text-xs bg-white border border-zinc-200 rounded-xl focus:outline-none focus:border-indigo-500 font-semibold" required />

                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col gap-1">
                        <span className="text-[9px] font-black text-zinc-400">Bobot SKS:</span>
                        <select value={newCourseCredits} onChange={(e) => setNewCourseCredits(e.target.value)} className="p-2 text-xs bg-white border border-zinc-200 rounded-xl text-zinc-600 font-bold">
                          <option>2</option>
                          <option>3</option>
                          <option>4</option>
                        </select>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select value={newCourseGrade} onChange={(e) => setNewCourseGrade(e.target.value)} className="p-2 text-xs bg-white border border-zinc-200 rounded-xl text-zinc-600 font-bold">
                          <option>A</option>
                          <option>A-</option>
                          <option>B+</option>
                          <option>B</option>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="p-4 bg-white border border-zinc-150 rounded-2xl">
                      ...
                      <p className="text-[10px] font-semibold text-zinc-500 leading-relaxed mt-1">{calculateGpa() >= 3.5 ? "🎉 Pertahankan! IP Anda saat ini telah memenuhi syarat kualifikasi kelulusan predikat Cum Laude UI." : "💡 Target belum tercapai. Tingkatkan nilai A Anda pada sisa 4 mata kuliah berikutnya."}</p>
                    </div>
                          ...
                          grades.map((g) => (
                            <div key={g.id} className="p-2 bg-white border border-zinc-150 rounded-xl flex justify-between items-center text-[10px] font-bold">
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                ...
                                <button onClick={() => handleDeleteGrade(g.id)} className="w-5 h-5 flex items-center justify-center text-zinc-300 hover:text-red-500 transition rounded" title="Hapus">
                                  ✕
                                </button>
                              </div>
                            </div>
            ...
            {activeApp === "calendar" && (
              <div className="flex flex-col gap-6">
                <div className="border-b border-black/5 pb-5">
                  <h2 className="text-lg font-semibold text-[#1D1D1F] tracking-tight">Reminders & Schedules</h2>
                  <p className="text-[13px] text-[#86868B] mt-1 font-medium">Keep track of your upcoming academic milestones.</p>
                </div>

                <div className="flex flex-col gap-6">
                  {/* Countdown Jumbotron */}
                  <div className="p-8 bg-zinc-50 border border-black/5 rounded-3xl text-center flex flex-col items-center justify-center">
                    <span className="text-[10px] font-bold tracking-widest text-[#86868B] uppercase">Upcoming Milestone</span>
                    <h3 className="text-2xl sm:text-3xl font-medium mt-3 tracking-tight text-[#1D1D1F]">{timeLeft || "No schedules found"}</h3>
                  </div>

                  {/* Form to add reminder */}
                  <form onSubmit={handleAddReminder} className="p-6 bg-white border border-black/5 shadow-[0_4px_20px_rgb(0,0,0,0.03)] rounded-3xl flex flex-col gap-4">
                    <h3 className="text-[13px] font-semibold text-[#1D1D1F]">New Reminder</h3>
                    <input type="text" placeholder="Title (e.g. Database Systems Midterm)" value={newReminderTitle} onChange={(e) => setNewReminderTitle(e.target.value)} className="p-3 text-[13px] bg-zinc-50 border border-black/5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-black/10 focus:bg-white transition-all font-medium text-[#1D1D1F] placeholder:text-[#86868B]" required />
                    <div className="grid grid-cols-2 gap-3">
                      <input type="date" value={newReminderDate} onChange={(e) => setNewReminderDate(e.target.value)} className="p-3 text-[13px] bg-zinc-50 border border-black/5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-black/10 focus:bg-white transition-all font-medium text-[#1D1D1F] placeholder:text-[#86868B]" required />
                      <select value={newReminderPriority} onChange={(e) => setNewReminderPriority(e.target.value as any)} className="p-3 text-[13px] bg-zinc-50 border border-black/5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-black/10 focus:bg-white transition-all text-[#1D1D1F] font-medium">
                        <option value="HIGH">High Priority</option>
                        <option value="MID">Medium Priority</option>
                        <option value="LOW">Low Priority</option>
                      </select>
                    </div>
                    <button type="submit" className="w-full mt-2 py-3 bg-[#0071E3] hover:bg-[#0077ED] text-white text-[13px] font-semibold rounded-2xl transition-colors">
                      Add to Schedule
                    </button>
                  </form>
                  ...
                  {/* Milestones List */}
                  <div className="flex flex-col gap-3">
                    <h3 className="text-[13px] font-semibold text-[#86868B] px-1">Your Schedule</h3>

                    {reminders.length === 0 ? (
                      <div className="py-10 text-center border-t border-black/5">
                        <p className="text-[13px] text-[#86868B] font-medium">No schedule added yet.</p>
                      </div>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {reminders.map((r) => (
                          <div key={r.id} className="p-4 bg-white border border-black/5 shadow-[0_2px_10px_rgb(0,0,0,0.02)] rounded-3xl flex items-center gap-4 text-xs justify-between transition-all hover:shadow-[0_4px_12px_rgb(0,0,0,0.03)] hover:-translate-y-0.5">
                            <div className="flex items-center gap-4">
                              <span className={`w-2 h-2 rounded-full shrink-0 ${r.priority === "HIGH" ? "bg-red-500" : r.priority === "MID" ? "bg-orange-500" : "bg-[#0071E3]"}`} />
                              <div className="flex-1">
                                <h4 className="text-[#1D1D1F] font-medium text-[13px] tracking-tight">{r.title}</h4>
                                <p className="text-[11px] text-[#86868B] mt-1 font-medium">{new Date(r.date).toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold ${r.priority === "HIGH" ? "bg-red-50 text-red-600" : r.priority === "MID" ? "bg-orange-50 text-orange-600" : "bg-blue-50 text-[#0071E3]"}`}>{r.priority === "HIGH" ? "High" : r.priority === "MID" ? "Medium" : "Low"}</span>
                              <button type="button" onClick={() => handleDeleteReminder(r.id)} className="w-7 h-7 flex items-center justify-center rounded-full bg-zinc-50 hover:bg-zinc-100 text-[#86868B] hover:text-[#1D1D1F] transition-colors" title="Delete">
